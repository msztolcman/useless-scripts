...
def compare_files (f1, f2, return_diff = False):
  ...
  f1_content = open (f1, 'r').readlines ()
  f2_content = open (f2, 'r').readlines ()
        ...
        if create_diff and not write_error:
          try:
            ...
            if not os.path.isdir (dirs):
              os.makedirs (dirs, 0o755)
            fh = open (os.path.join (dirs, os.path.basename (f) + g_suffix), 'w')
            fh.writelines (diff)
            fh.close ()
          except:
            write_error = True
